...
try {
  ...
  try {
    ...
    const resolved = require.resolve("zod");
    zMod = await import(pathToFileURL(resolved).href);
  } catch {
    // Fallback schema validator for standalone runners
  }
}
...
const z: any = zMod?.z ?? zMod?.default?.z ?? zMod?.default ?? new SchemaFallback();
...
export type EvidenceStatus = "verified" | "assumption" | "missing";
...
export type FeasibilityReport = {
  ...
  killCriteria: string[];
  limitations: string[];
  financial: ReturnType<typeof buildFinancialModel>;
};
