...
export const lookupYorkAddress = createServerFn({ method: "POST" })
  ...
  .validator((input: unknown) => z.object({ q: z.string().min(4).max(160) }).parse(input))
  .handler(
    async ({
      data,
      context,
      ...
      if (!match?.coordinates) {
        return {
          ok: false,
          error: "No Census match. Try a full street address in York County, PA.",
        };
      }
      ...
      const [parcelJson, zoningJson] = await Promise.all([
