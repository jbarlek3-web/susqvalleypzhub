...
import { clerkAuthorizedParties } from "../clerk-authorized-parties";
...
export function assertSameSiteRequest(explicitRequest?: Request): void {
  const request = explicitRequest ?? getRequest();
  ...
  // Defense-in-depth: when Origin is present, reject untrusted cross-origin requests
  ...
  if (origin) {
    ...
    if (process.env.APP_URL?.trim()) {
      try {
        configuredOrigin = new URL(process.env.APP_URL.trim()).origin;
      } catch {
    ...
    if (
      ...
      const allowed = clerkAuthorizedParties(process.env);
