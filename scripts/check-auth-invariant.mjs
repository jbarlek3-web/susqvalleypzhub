...
async function main(argv) {
  ...
  const explicitDevUrl = devUrlFlag !== -1;
  const devUrl = explicitDevUrl ? argv[devUrlFlag + 1] : DEFAULT_DEV_URL;
  const devAuth = await probeDevAuthEnabled(devUrl);

  if (devAuth === null && !explicitDevUrl) {
    const buildAuth = buildAuthEnabled();
    console.log(
      `[auth-invariant] dev server not running; build auth verified: sign-in ${buildAuth ? "on" : "off"}`,
    );
    process.exit(0);
  }
